...
import { searchArticle } from '@/_services/ArticleService'; // API call function
import SearchBar from '../components/SearchBar.vue';

// Reactive Variables
const articles = ref<{ titre: string; created_at: string; contenu: string; note: number }[]>([]);
...
const fetchArticles = async () => {
  try {
    const response = await searchArticle(titre.value); // Assuming API function takes a title parameter
    articles.value = response.data; // Adjust based on API response format
    error.value = null;
  } catch (err) {
...
<template>
  <main>
    <div class="form-container my-5 py-5">
      ...
      <!-- Articles -->
      <div v-for="(article, index) in articles" :key="index" class="carte container d-none d-lg-block my-5">
        <div class="row">
